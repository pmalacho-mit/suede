...
class gitrepo:
    ...
    def read_manifest(directory: str) -> Manifest:
        manifest_dir = os.path.join(directory, MANIFEST_DIR)
        ...
        for filename in sorted(os.listdir(manifest_dir)):
            ...
            if pin:
                edges[filename[: -len(GITREPO)]] = pin
        return Manifest(edges=edges, npm=npm.declared_in(manifest_dir))
...
def plan(
    ...
    return Plan(
        ...
        warnings=tuple(warnings) + _situational_warnings(world, request, resolution),
