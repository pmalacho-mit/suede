...
- **`core/`** — suede-specific helper files that get vendored into the dependency.
  - On `release`, these ship to **consumers** (see [release/core/README.md](./release/core/README.md)): [`upstream`](./release/core/upstream), which proposes a consumer's local changes back to the library as a PR, [`sync`](./release/core/sync), which pulls one, and [`diff`](./release/core/diff), which shows what either of those would move.
